...
export const shareNative = async (shareText: string): Promise<boolean> => {
    if (/Android|iPhone|iPad|iPod/i.test(navigator.userAgent))
        return await shareSMS(shareText);
 ...
 */
export const shareSMS = async (shareText: string): Promise<boolean> => {
    ...
    if (navigator.share) {
        try {
            ...
            if (error instanceof DOMException && !error.toString().includes('AbortError'))
 ...
 */
export const shareToClipboard = async (shareText: string): Promise<boolean> => {
    let success = false;
